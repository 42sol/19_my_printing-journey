...
module base_shape(){
    difference(){
        union(){
          rounded_cube([200,36,20],5,0,1);
            difference(){
            translate([0,-3,0]) rounded_cube([8,42,29],5,1,1);
            translate([6,-4,-1]) cube([5,43,33]);
            }
        }
        translate([100,18,0]) rotate([0,0,-90]) mmark();
        ...
        // die spots
        translate([0,18.5,12]) import("mydice.stl");
        // grooves for top
        translate([-1,-1,7]) cube([200,4,2]);
        translate([-1,33,7]) cube([200,4,2]);
        translate([197,0,7]) cube([4,40,2]);
        // magnet pocket
...
module cover_shape(){
   difference(){
        translate([-1,-48,0])rounded_cube([205,42,22],6,1,0);
        translate([-5,-46,1]) rounded_cube([208,38,19],5,1,0);
        translate([-1,-43,-1]) cube([197.5,32,14]);
        translate([-7,-48.5,-1]) cube([13,43,34]);
        //translate([202,-27,9]) rotate([0,90,0]) cylinder(d=15,h=2.5); //magnet
    }
    
   color("black")translate([100,-27,21.8])linear_extrude(1.0) text(toptext,font = "Liberation Sans:style=Bold",halign="center",valign="center");
...
if ( display == "all" || display == filler) translate([0,-50,21]) rotate([180,0,0])filler();
